...
def lookup(model: str, prompt: str):
    inputs = [] # list to store incoming contents of the prompt
    responses = [] # list to store the pre-generated responses appropriate for the response to the request

    if prompt == None or prompt == "":
        inputs.append(prompt) # adds either "None" or "", both of which are keys in the response cache dictionary
    ...
    # "None" causes issues for the "in" operator, so only continue checks if the prompt wasn't "None"
        ...
        if re_compile(r'[\u4E00-\u9FFF]').findall(lowercase_prompt) != []: # if there were chinese characters in the prompt:
            ...
            inputs.append("chinese") # any chinese characters present will result in the same response (a summary of the movie "Farewell My Concubine" in Chinese)
